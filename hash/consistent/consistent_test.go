...
func TestNew(t *testing.T) {
	...
	checkNum(x.replicas, constReplicas, t)
}

func TestAdd(t *testing.T) {
	x := NewConsistent()
	x.Add("abcdefg")
	checkNum(len(x.virtualMap), constReplicas, t)
	checkNum(len(x.circle), constReplicas, t)
	if sort.IsSorted(x.circle) == false {
		t.Error("expected sorted hashes to be sorted")
	}
	x.Add("qwer")
	checkNum(len(x.virtualMap), constReplicas*2, t)
	checkNum(len(x.circle), constReplicas*2, t)
	if sort.IsSorted(x.circle) == false {
		t.Error("expected sorted hashes to be sorted")
	}
}
...
// 此测试用于检测虚拟节点的映射关系
func TestAdd_JiangCheng(t *testing.T) {
	x := NewConsistent()
	for i := 0; i < constReplicas; i++ {
		x.Add(fmt.Sprintf("Machine%2d", i))
	}

	for index, v := range x.virtualMap {
		t.Logf("%32d--->%10s \n", index, v)
	}
...
func TestSet(t *testing.T) {
	...
	if x.GetMachineNum() != 2 {
		t.Errorf("expected 2 elts, got %d", x.GetMachineNum())
	}
